...
- 答**一个系统** → 单人档：删掉 AGENTS.md 里所有标「单人开发：删掉」和「单流：删掉」的条款（标「分流：删掉」的留着——那是单流自己的写法）。到此为止，不问第二问。
...
3. 按第 2 步的答案删掉不适用那一档的条款（标「单人开发：删掉」/「单流：删掉」/「分流：删掉」的）。
4. 目录约定与本仓不同时改路径——**改了要记下来**，`sdd-loop check` 得用 `--status-file` / `--archive-dir` 指过去，否则它判的是默认路径。

**四种标记别一起删。**「源项目迁移」说的是要**加**什么（默认不在），「单人开发：删掉」「单流：删掉」「分流：删掉」说的是要**删**什么（默认就在）。

**状态文件路径那一对是互斥的，必须删掉其中一段。**「Loop 状态入口」和「文档规则」里各有一对：一段写单流的路径（标「分流：删掉」），一段写分流的路径（标「单流：删掉」）。两段都留下，这份门禁就同时声明了两个状态入口——读它的人不知道该信哪个，而门禁文件里没有「大概」这一说。这一对是**唯一**「不删就出错」的地方，别的标记漏删只是多几句用不上的话。而**没有标记的条款一律常驻**，任何项目都不许删——尤其「数据模型变更」和「确认留痕」这两条，前者是每轮属性（只要还有库就会回来），后者单人时看着像废话但同样是每轮属性（三个月后自己也分不出那个 `confirmed` 确认的是哪一版）。模板顶部的注释块把这三档写全了，动手前读一遍。
