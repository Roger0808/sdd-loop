...
const REQUIRED_FIELDS = [
  "document", "hotfixId", "status", "hotfixState", "reviewRoute", "baseBranch", "baseCommit",
...
export function buildHotfixChecks(scan) {
  ...
  const h2 = check("H2", "用户授权、项目角色与 reviewer 路径完整");
  const h3 = check("H3", "修复、回滚、工程扩展与架构证据对应当前指纹");
  const h4 = check("H4", "AI Review 与人工签署对应当前指纹");
  ...
  for (const file of hotfixes.files) {
    ...
    if (!ROUTES.has(meta.reviewRoute)) fail(h2, `${file.path} 必须选择 current-subagent 或 external-agent reviewer 路径。`, { file: file.path });
    ...
    if (file.location === "archive") {
      ...
      const closed = latest(events, "hotfix_closed");
      if (!closed || closed.codeFingerprint !== meta.fixFingerprint) fail(h5, `${meta.hotfixId} 缺少归档指纹对应的 hotfix_closed。`, { file: file.path });
    }
